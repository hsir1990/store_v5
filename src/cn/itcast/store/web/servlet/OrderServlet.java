...
public class OrderServlet extends BaseServlet {
	...
	public String saveOrder(HttpServletRequest request, HttpServletResponse response)
		...
		for(CartItem item : cart.getCartItems()) {
			...
			//设置当前的订单属于哪个订单：程序的角度体验订单项和订单对应关系
	...
	public String findMyOrdersWithPage(HttpServletRequest request, HttpServletResponse response)
		...
		//PageModel: 1_分页参数 2_url 3_当前用户的当前页的订单(集合)，每笔订单上对象的订单项，以及订单项对应的商品信息
	...
	public String callBack(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(isValid) {
			...
			if(r9_BType.equals("1")) {
				...
				//如果支付成功，更新订单状态
				OrderService orderService = new OrderServiceImp();
				Order order = orderService.findMyOrderByOid(r6_Order);
				order.setState(2);
				orderService.updateOrder(order);
				//向request放入提示信息
